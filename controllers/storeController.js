const express = require('express');
    ...
    router.post('/stores', verifyToken, (req, res) => {
      ...
      db.run(sql, [storeId, name, req.userId, activationKey], function(err) {
        if (err) {
    ...
    router.put('/stores/:id', verifyToken, (req, res) => {
      ...
      db.run(sql, [name, id, req.userId], function(err) {
        if (err) {
          console.error('Error updating store:', err);
          return res.status(500).send('Error updating store.');
        }
        this.changes === 0 ? res.status(404).send('Store not found or unauthorized.') : res.status(200).send('Store updated successfully.');
      });
    });
    ...
    router.delete('/stores/:id', verifyToken, (req, res) => {
      ...
      db.run(sql, [id, req.userId], function(err) {
        ...
        this.changes === 0 ? res.status(404).send('Store not found or unauthorized.') : res.status(200).send('Store deleted successfully.');
      });
    });
    ...
    router.post('/stores/activate', (req, res) => {
      ...
      db.run(sql, [activationKey], function(err) {
        if (err) {
          console.error('Error activating store:', err);
          return res.status(500).send('Error activating store.');
        }
        this.changes === 0 ? res.status(404).send('Invalid activation key.') :
        db.get('SELECT * FROM stores WHERE activation_key = ?', [activationKey], (err, store) => {
          if (err) {
    ...
    router.get('/stores/me', verifyToken, (req, res) => {
      ...
      db.all(sql, [req.userId], (err, results) => {
        if (err) {
