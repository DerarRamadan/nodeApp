...
router.post('/programs', verifyToken, upload.single('dbFile'), (req, res) => {
  ...
  db.run(sql, [name, fileName, fileType, fileSize], function(err) {
    if (err) {
...
router.get('/programs/query/:programId', (req, res) => {
  ...
  db.get(sql, [programId], (err, result) => {
    if (err) {
    ...
    const { name, db_file: fileName, file_type: fileType, file_size: fileSize } = result;
    const filePath = path.join(queriesDir, fileName);
